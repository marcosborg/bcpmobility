...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vehicleMaintenance.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vehicle-maintenances.update", [$vehicleMaintenance->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('car_item') ? 'is-invalid' : '' }}" name="car_item_id" id="car_item_id" required>
                    @foreach($car_items as $id => $entry)
                        <option value="{{ $id }}" {{ (old('car_item_id') ? old('car_item_id') : $vehicleMaintenance->car_item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" type="text" name="reason" id="reason" value="{{ old('reason', $vehicleMaintenance->reason) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_time_start') ? 'is-invalid' : '' }}" type="text" name="date_time_start" id="date_time_start" value="{{ old('date_time_start', $vehicleMaintenance->date_time_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_time_end') ? 'is-invalid' : '' }}" type="text" name="date_time_end" id="date_time_end" value="{{ old('date_time_end', $vehicleMaintenance->date_time_end) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('observations') ? 'is-invalid' : '' }}" name="observations" id="observations">{{ old('observations', $vehicleMaintenance->observations) }}</textarea>
