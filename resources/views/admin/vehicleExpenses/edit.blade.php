    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.vehicle-expenses.update", [$vehicleExpense->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('car_item') ? 'is-invalid' : '' }}" name="car_item_id" id="car_item_id" required>
                    @foreach($car_items as $id => $entry)
                        <option value="{{ $id }}" {{ (old('car_item_id') ? old('car_item_id') : $vehicleExpense->car_item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\VehicleExpense::EXPENSE_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('expense_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="expense_type_{{ $key }}" name="expense_type" value="{{ $key }}" {{ old('expense_type', $vehicleExpense->expense_type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $vehicleExpense->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $vehicleExpense->description) !!}</textarea>
