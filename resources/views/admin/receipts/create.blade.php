    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.receipts.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('verified_value') ? 'is-invalid' : '' }}" type="number" name="verified_value" id="verified_value" value="{{ old('verified_value', '0') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_transferred') ? 'is-invalid' : '' }}" type="number" name="amount_transferred" id="amount_transferred" value="{{ old('amount_transferred', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Receipt::COMPANY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('company') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="company_{{ $key }}" name="company" value="{{ $key }}" {{ old('company', 'OC') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Receipt::IVA_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('iva') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="iva_{{ $key }}" name="iva" value="{{ $key }}" {{ old('iva', 'Sem IVA') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Receipt::RETENTION_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('retention') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="retention_{{ $key }}" name="retention" value="{{ $key }}" {{ old('retention', '11.50%') === (string) $key ? 'checked' : '' }} required>
